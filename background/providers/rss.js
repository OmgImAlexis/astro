...
let start, end, getProtocol, getCategory, getRequestOptions, getFeedURL, getTorrentInfo;
...
function parse(err, body, feedURL, callback) {
    ...
    parseString(body).then(result => { // eslint-disable-line complexity
...
class RSS extends Provider {
    constructor(options) {
        ...
        getTorrentInfo = require('../get-torrent-info');
        start = this.start;
        end = this.end;
    }
